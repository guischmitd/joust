...
def buchholz_difficulty_score(contestant: Contestant, history: list[Match], registry: Registry) -> int:
    """Buchholz: sum of (wins - losses) of all opponents faced."""
    score = 0
    for m in history:
        if m.left_wins is None:
            continue
        if m.left == contestant.team:
            opp_team = m.right
        elif m.right == contestant.team:
            opp_team = m.left
        else:
            continue
        opp = registry[opp_team]
        score += opp.wins - opp.losses
    return score


def recalculate_buchholz_seeds(contestants: list[Contestant], history: list[Match], registry: Registry) -> None:
    """Re-seed in place by: W-L record (desc), Buchholz (desc), initial seed (asc)."""
    scores = {c: buchholz_difficulty_score(c, history, registry) for c in contestants}
    contestants.sort(
        key=lambda c: (
            -(c.wins - c.losses),
            -scores[c],
...
class ValveBuchholzMatchup:
    ...
    def update_seeds(
        ...
        recalculate_buchholz_seeds(contestants, match_history, registry)
    ...
    def __round_2_matchups(self, sorted_contestants: list[Contestant]) -> list[Match]:
        ...
        return [
            ...
            for left, right in zip(
                sorted_contestants[:halfpoint],
                reversed(sorted_contestants[halfpoint:]),
            )
        ]
    ...
    def _get_matchups_for_group(self, contestants: list[Contestant]) -> list[Match]:
        ...
        assert False, f"Unhandled round: {self.round}"
